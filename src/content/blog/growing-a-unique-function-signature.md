...
None of these pieces are new on their own, and I want to be upfront about that. The exact, wildcard-free version of "shortest unique substring starting here" is a studied problem in string algorithms, called left-bounded shortest unique substring, with linear-time suffix-array solutions. The masked version touches the literature on pattern matching with wildcards. Inverted byte buckets, seed-and-verify, monotone candidate filtering: all standard.
